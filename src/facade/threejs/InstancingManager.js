...
import {upgradeVertexShader} from './InstancingShaderUpgrades'
...
class InstancingManager extends Group3DFacade {
  ...
  _getBatchObject(instancedObject) {
    ...
    batchMaterial.onBeforeCompile = onBeforeCompileBatchMaterial
...
function onBeforeCompileBatchMaterial(shaderInfo) {
  // this === the batch material
  let baseMaterial = this.$troikaBatchBase
  let vertShader = baseMaterial.$troikaUpgradedVertex
  if (!vertShader || vertShader.original !== shaderInfo.vertexShader) {
    vertShader = baseMaterial.$troikaUpgradedVertex = {
      original: shaderInfo.vertexShader,
      upgraded: upgradeVertexShader(shaderInfo.vertexShader)
    }
  }
  shaderInfo.vertexShader = vertShader.upgraded
}
      ...
      for (let id in pools) {
        ...
        for (let i = firstFree, len = geometries.length; i < len; i++) {
          ...
          if (firstFree > 0) {
            let instancingAttrs = {}
            for (let row = 0; row < 3; row++) {
              let name = `troika_modelMatrixRow${row}`
