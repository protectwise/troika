...
/* global Ammo, CONSTANTS, utils */
'use strict'

importScripts('engines/PhysicsEngineBase.js')

let sharedTransform = null

class AmmoPhysicsEngine extends self.PhysicsEngineBase {
  constructor (params) {
    super(params)
    ...
    if (!self.ammoShapeManager) {
      throw new Error('AmmoPhysicsEngine requires an ammoShapeManager')
    }
  ...
  _init () {
    ...
    sharedTransform = new Ammo.btTransform()
  ...
  _addRigidBody (facadeId, bodyConfig) {
    ...
    const physicsShape = self.ammoShapeManager.getShape(shapeConfig)
  ...
  remove (facadeId) {
    ...
    // self.ammoShapeManager.objectRemoved()
    super.remove(facadeId)
  }
    ...
    for (const facadeId in this._rigidBodies) {
      ...
      if (physicsBody.isActive()) {
        ...
        if (motionState) {
          ...
          motionState.getWorldTransform(sharedTransform)
          var pos = sharedTransform.getOrigin()
          facadeOutput[1] = pos.x()
          facadeOutput[2] = pos.y()
          facadeOutput[3] = pos.z()
          var quat = sharedTransform.getRotation()
...
self.AmmoPhysicsEngine = AmmoPhysicsEngine
